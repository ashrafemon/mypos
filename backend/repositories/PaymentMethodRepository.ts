...
import { StoreRules, UpdateRules } from "../rules/paymentMethods";
...
export default class PaymentMethodRepository {
    ...
    async index(queries: any) {
        ...
        if (queries.get_all && Number(queries.get_all) === 1) {
            const docs = await this.db.paymentMethod.findMany({
                select: fields,
                where: condition,
            });
            ...
            const [count, docs] = await this.db.$transaction([
                this.db.paymentMethod.count({ where: condition }),
    ...
    async show(id: string, queries: any) {
        ...
        const doc = await this.db.paymentMethod.findFirst({
            where: condition,
            select: fields,
        });
        if (!doc) {
    ...
    async update(id: string, body: any) {
        ...
        const doc = await this.db.paymentMethod.findFirst({ where: condition });
        if (!doc) {
            return this.helper.errorResponse({
                message: "Payment method not found...",
            });
