...
import { StoreRules, UpdateRules } from "../rules/customerGroups";
import { DynamicObjectTypes } from "../types/baseTypes";

export default class CustomerGroupRepository {
    private helper;
    private db;
    ...
    async index(queries: any) {
        ...
        if (queries.get_all && Number(queries.get_all) === 1) {
            const docs = await this.db.customerGroup.findMany({
                select: fields,
                where: condition,
            });
            ...
            const [count, docs] = await this.db.$transaction([
                this.db.customerGroup.count({ where: condition }),
    ...
    async show(id: string, queries: any) {
        ...
        const doc = await this.db.customerGroup.findFirst({
            where: condition,
            select: fields,
        });
        if (!doc) {
    ...
    async update(id: string, body: any) {
        ...
        const doc = await this.db.customerGroup.findFirst({ where: condition });
        if (!doc) {
            return this.helper.errorResponse({
                message: "Customer group not found...",
            });
