...
import { StoreRules, UpdateRules } from "../rules/incomeCategories";
...
export default class IncomeCategoryRepository {
    ...
    async index(queries: any) {
        ...
        if (queries.get_all && Number(queries.get_all) === 1) {
            const docs = await this.db.incomeCategory.findMany({
                select: fields,
                where: condition,
            });
            ...
            const [count, docs] = await this.db.$transaction([
                this.db.incomeCategory.count({ where: condition }),
                this.db.incomeCategory.findMany({
                    select: fields,
                    where: condition,
                    skip: offset - 1,
                    take: limit,
    ...
    async show(id: string, queries: any) {
        ...
        const doc = await this.db.incomeCategory.findFirst({
            where: condition,
            select: fields,
        });
        if (!doc) {
    ...
    async update(id: string, body: any) {
        ...
        await this.db.incomeCategory.update({
            where: { id: doc.id },
