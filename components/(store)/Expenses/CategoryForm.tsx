...
} from "@/states/actions/stores/expenseCategories";
...
const CategoryForm: React.FC<{
    isFetching?: boolean;
    data?: ExpenseCategoryType | null;
    close: () => void;
}> = ({ isFetching = false, data, close = () => {} }) => {
    const [create, result] = useCreateExpenseCategoryMutation();
    const [update, resultUpdate] = useUpdateExpenseCategoryMutation();
    ...
    useEffect(() => {
        if (data && Object.keys(data).length > 0) {
            const payload: ExpenseCategoryType = { ...data };
            let obj = { ...form };
