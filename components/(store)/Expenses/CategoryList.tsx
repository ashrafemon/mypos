...
const CategoryList = () => {
    ...
    const { data, isFetching, isError, error } = useFetchExpenseCategoriesQuery(
        `offset=${queries.page}&limit=${queries.offset}${
            queries.search ? `&search=${queries.search}` : ""
        }`
    );

    const [deleteCategory, result] = useDeleteExpenseCategoryMutation();
    ...
    } = useFetchExpenseCategoryQuery(selectedId, {
        skip: !selectedId,
